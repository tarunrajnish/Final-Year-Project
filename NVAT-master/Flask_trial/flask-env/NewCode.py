...
class DetectAndSave:
	...
	def create_video_clip(self, file_name, width, height):
		...
		file_path = "static/video_clips/{}".format(file_name.replace(':', '-'))
	...
	def detect(self, camera, file, start_time):
		...
		while True:
			...
			if grabbed == True:
				...
				if (frame_count % 1*self.frames_per_second == 0):	
					...
					Intruder.check_active(frame_count, self.current_time, video_id,False)
				...
				for c in cnts:
					...
					if (self.current_time.time() > date.time() and self.current_time.time() < date1.time()) \
						...
						if x > 30 and y > 50 and h > 20 and w > 20 and y < 500 and x < 700:
							#xCenter=x + w/2;
							#yCenter=y + h/2;
							...
							if (intruder.new and Intruder.get_count() == 1):
								...
								file_name = "Video_{}_{}.avi".format(camera.id, self.current_time)
							...
							if (intruder.new  ):
								...
								frame_file = "static/intruder_profiles/cropFrame_{}.jpg".format(intruder.data.id)
								frame_file1 = "static/intruder_profiles/fullFrame_{}.jpg".format(intruder.data.id)
			...
			# cleanup the camera and close any open windows
				...
				Intruder.check_active(frame_count, self.current_time, video_id,True)
				print "exit"
				break
...
if __name__ == '__main__':
	...
	try:
		...
		if (camera is None):
			print "Camera with id: %d not found" % int(camera_id)
			exit(-1)
		...
		start_time = datetime.strptime(tokens[len(tokens) - 1], "%Y-%m-%d %H-%M-%S")
		...
		#video='vid.mp4'
		d.detect(camera, file, start_time)
	
	except Exception as e:
		logging.exception("Error in NewCode")